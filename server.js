...
app.post('/api/send-certificates', requireAuth, async (req, res) => {
  ...
  const studResult = await pool.query(
    'SELECT * FROM students WHERE id = ANY($1::text[])',
    [studentIds]
  );
  ...
  for (const student of studResult.rows) {
    try {
      ...
      await transporter.sendMail({
        from: `"${s.school}" <${process.env.GMAIL_USER}>`,
        to: student.email,
        subject: `Certificate of Completion — ${courseName} — Triangle Real Estate Academy`,
        text: emailBody,
        attachments: [{
...
function generateCertPDFBuffer(student, course, s) {
  ...
  const courseShort = course.name
    .replace(/^Post-License\s*/,'')
    .replace(/^Elective:\s*/,'')
    .replace(/\sv\.\d+$/,'').trim() || course.name;

  // Borders
  doc.setDrawColor(44,44,42);
  ...
  const addrLines = [s.addr1, s.addr2, 'Phone: '+s.phone, 'Email: '+s.email, 'Web: '+s.web];
  ...
  const totalW = pw-80, colW = totalW/fields.length;
  fields.forEach((f,i) => {
  ...
  doc.setLineWidth(0.4);
  doc.line(148, 153, pw-margin-8, 153);
  doc.setFontSize(8);
  doc.setTextColor(120,120,120);
  doc.text('Instructor / Code', pw-70, 158, {align:'center'});

  // Signature (text-based since we don't have image on server)
  doc.setFont('times','italic');
  doc.setFontSize(16);
  doc.setTextColor(40,40,40);
  doc.text('Keith E. Green', 70, 178, {align:'center'});
  doc.setLineWidth(0.4);
  doc.setDrawColor(0,0,0);
  doc.line(margin+8, 181, 130, 181);
  doc.setFont('helvetica','normal');
  doc.setFontSize(8);
  doc.setTextColor(120,120,120);
  doc.text('Education Director Signature', 70, 186, {align:'center'});

  // Date
  doc.setFont('helvetica','normal');
  doc.setFontSize(11);
  doc.setTextColor(0,0,0);
  doc.text(fmtDate(course.end_date), pw-70, 178, {align:'center'});
  doc.setLineWidth(0.4);
  doc.setDrawColor(0,0,0);
  doc.line(148, 181, pw-margin-8, 181);
  doc.setFontSize(8);
  doc.setTextColor(120,120,120);
  doc.text('Date', pw-70, 186, {align:'center'});
...
app.post('/api/ncrec-roster', requireAuth, async (req, res) => {
  ...
  res.setHeader('Content-Disposition', `attachment; filename="NCREC_Roster_${course.num}_${course.start_date}.txt"`);
  res.send(lines.join('\r\n'));
});
