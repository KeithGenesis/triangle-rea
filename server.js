...
app.post('/api/send-certificates', requireAuth, async (req, res) => {
  ...
  const senderEmail = s.email || process.env.GMAIL_USER;

  for (const student of studResult.rows) {
    if (!student.email) {
      results.failed.push({ name: student.name, error: 'No email address on file' });
      continue;
    }
    try {
      ...
      // Build RFC 2822 MIME message for Gmail API (avoids SMTP port blocking)
      const filename = student.name.replace(/[^a-zA-Z0-9]/g, '_') + '_Certificate.pdf';
      const boundary = 'boundary_' + Date.now();
      const pdfB64 = pdfBuffer.toString('base64');

      const mimeMessage = [
        `From: "${s.school}" <${senderEmail}>`,
        `To: ${student.email}`,
      ...
      // Send via Gmail API (HTTPS on port 443 — not blocked by Railway)
      ...
      if (!gmailRes.ok) {
        ...
        throw new Error(errBody.error?.message || `Gmail API error ${gmailRes.status}`);
      }
